...
public class OrderserviceImpl extends AbstractComponentFacade implements Orderservice {
  ...
  public ItemEto updatePriceByItemsName(String name, Double price) {
    ...
    return this.ucManageItem.updatePriceByItemsName(name, price);
  }
  ...
  public Collection<OrderEto> findOrdersFromDateWithStatus(Date fromDate, OrderStatus status) {
    ...
    return this.ucFindOrder.findOrdersFromDateWithStatus(fromDate, status);
  }

  @Override
  public OrderCto createOrderWithOrderPositionsAndOwner(Set<ItemEto> orderPositions, CustomerEto owner) {

    return this.ucManageOrder.createOrderWithOrderPositionsAndOwner(orderPositions, owner);
  }
