...
public class UcManageOrderImpl extends AbstractOrderUc implements UcManageOrder {
  ...
  public OrderCto createOrderWithOrderPositionsAndOwner(Set<ItemEto> orderPositions, CustomerEto owner) {
    ...
    Set<ItemEntity> itemEntities = orderPositions.stream().map(item -> getBeanMapper().map(item, ItemEntity.class))
        .collect(Collectors.toSet());

    OrderEntity resultEntity = getOrderRepository().createOrderWithOrderPositionsAndOwner(itemEntities, ownerEntity);
